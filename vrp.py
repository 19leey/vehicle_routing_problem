...
class Load:
    def __init__(self, load_number: int, pickup: Point, dropoff: Point):
        ...
        self.duration = self.pickup.get_drive_time(
            self.dropoff
        )  # Drive time from pickup to dropoff
    ...
    while incomplete_loads:
        ...
        while True:
            ...
            if nearest_load is None:
                ...
                total_drive_time
                + drive_time
                + nearest_load.duration
                + nearest_load.get_drive_time(DEPOT)
            ) > MAX_SHIFT_TIME:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "path_to_problem", help="Path to text file containing VRP problem"
    )
